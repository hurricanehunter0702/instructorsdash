...
class SmsTemplateController extends Controller
{
    ...
    {
        $user = auth()->user();
        $smsHire = SmsHire::all()->sortBy('amount')->first();
        $smsHires = SmsHire::where("is_active", 1)->get()->sortBy('amount');
        $reminders = SmsReminderTypes::all();
        $smsAccount = SmsAccount::first();
        $messages = [];
        $errors = session()->get('errors', app(ViewErrorBag::class));
        if(!empty($errors->all())) {
            array_push($messages, $errors->all()[0]);
        }
        if ($user->sms_balance <= $smsAccount->sms_charge_limit) {
            array_push($messages, "You need to charge your SMS balance to use this feature.");
        }
    ...
    public function store(Request $request)
    {
        ...
        if (strlen(strip_tags($request->description)) > 200) {
            return redirect()->route("sms.user.templates.index", ["tab" => $data["reminder_id"]])->withErrors([
                "msg" => "Message text must be up to 200 characters."
            ]);
        } else {
