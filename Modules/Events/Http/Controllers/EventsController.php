...
class EventsController extends Controller
{
    ...
    {
        $now = Carbon::now();
        $data = Event::query()->whereDate('end_date', '>=', $now)->with(['category'])->withCount('guests');
    ...
    public function expired(Request $request) {
        ...
        return view('events::events.expired', [
            'events' => $events
        ]);
    }
    ...
    public function store(EventStoreRequest $request)
    {
        ...
        $user = Auth::user();
        if ($request->has("template_id") && !is_null($request->template_id)) {
            $template = EventTemplate::find($request->template_id);
            $newEvent = $template->event->replicate();
    ...
    public function edit(Request $request, $id)
    {
        ...
        $current = Carbon::now();
        if (Carbon::parse($event->end_date)->lt($current)) {
    ...
    public function update(EventUpdateRequest $request, $id)
    {
        ...
        if (!isset($data['is_recur'])) $data['is_recur'] = 0;
        if ($data["is_recur"] == 1) {
    ...
    public function getGuestsByEventId(Request $request, $id) {
        $event = Event::with(['guests.sub_guests', 'guests.upsell_histories.upsell'])->find($id);
