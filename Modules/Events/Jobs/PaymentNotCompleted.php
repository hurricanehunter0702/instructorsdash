...
class PaymentNotCompleted implements ShouldQueue
{
    ...
    public function paymentNotCompleteToMainGuest() {
        Mail::send([], [], function($message) {
            ...
            $body = str_replace('%event_checkout_link%', '<a href="https://1da.sh/e/checkout/' . $this->guest->guest_code . '">Go to checkout page</a>', $body);
            if ($this->event->start_date) $body = str_replace('%event_start_date%', $this->event->start_date->format('Y-m-d H:i:s'), $body);
            $body = str_replace("%user_company_name%", $this->event->user->company, $body);
            $body = str_replace("%user_company_email%", $this->event->user->user_company_email, $body);
            $body = str_replace("%user_company_phone%", $this->event->user->user_company_phone, $body);
            $body = str_replace("%user_company_website%", $this->event->user->user_company_website, $body);
        ...
        if ($user->sms_status && $user->sms_balance > $this->sms_account->sms_fee) {
            ...
            $body = str_replace('%guest_ticket_currency%', $this->guest->ticket_currency, $body);
            $body = str_replace('%event_address%', $this->event->address, $body);
            $body = str_replace("%event_checkout_link%", "https://1da.sh/e/checkout/" . $this->guest->guest_code, $body);
